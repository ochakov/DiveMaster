...
- [x] Phase 1 — scaffold
- [ ] Phase 2 — pin deco golden values against a reference implementation. **In progress:** planner-style harness (`PlannerSim`, descent 18 m/min + ascent 9 m/min with off-gassing credit) + all 60 grid cells pinned in `GoldenNdlTest` (regenerate via `NdlTableGenerator` → `build/ndl-golden.txt`). External leg pending: Ev fills the Subsurface column in `docs/phase2-validation.md` (exact planner settings inside). Acceptance: ±2 min (≤5% over 100 min), never >1 min more permissive than reference.
